...
class Evaluator:
    def GetPascalVOCMetrics(self,
        ...
        for c in classes:
            ...
            TP_sum, FP_sum, FN_sum = np.sum(TP), np.sum(FP), npos - np.sum(TP)
            class_precision = TP_sum / (TP_sum + FP_sum) if (TP_sum + FP_sum) > 0 else 0.0
            class_recall = TP_sum / (TP_sum + FN_sum) if npos > 0 else 0.0

            acc_FP, acc_TP = np.cumsum(FP), np.cumsum(TP)
            rec = acc_TP / npos if npos > 0 else np.zeros_like(acc_TP)
            prec = np.divide(acc_TP, (acc_FP + acc_TP))
            # print(npos)
            # rec = acc_TP / npos
            if npos == 0:
                rec = np.zeros_like(acc_TP)  # Recall을 0으로 설정
            else:
                rec = acc_TP / npos
            ...
            # print (f'Class={c}, Total Positive={npos} TP={np.sum(TP)}, FP={np.sum(FP)}, Precision={mpre}, Recall={mrec}, AP={ap}')
            
            print (f'Class={c}, AP={ap}, (TP={np.sum(TP)}, FP={np.sum(FP)}, Precision={class_precision:.4f}, Recall={class_recall:.4f}')
            # print (f'Class={c}, Total Positive={npos} TP={np.sum(TP)}, FP={np.sum(FP)}, AP={ap}')
            ret.append(r)
        return ret
    ...
    def PlotPrecisionRecallCurve(self,
        ...
        if not results:
            ...
            return results  # 클래스가 없는 경우 바로 리턴

        for result in results:
            if result is None:
                continue  # None 값이 있는 경우 건너뛰기
